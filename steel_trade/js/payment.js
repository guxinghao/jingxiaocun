$(function(){
	...
	$(".unset").click(function(){
		$.cookie("check_payment_list",0,{path: '/'});
		$.cookie("check_payment_list_fybz",0,{path: '/'});
		$(".fixed-left .checkone").each(function(){
			$(this).attr("checked",false);
		})
	...
	$(".btn_submit").click(function(){
		var list = $.cookie("check_payment_list");
		if(typeof list == "undefined" || list == 0){
			confirmDialog("请选择明细!");
			return false;
		}
		$.get("/index.php/voucher/ProcessPaymentData",{},function(data){
			if(data == "success"){
				$.cookie("check_payment_list",0,{path: '/'});
				$.cookie("check_payment_list_fybz",0,{path: '/'});
...
//点击单个复选框处理函数
	function toggleCheck(obj){
		var id = obj.val();
		var bzmx=obj.attr('bz');
		var list = $.cookie("check_payment_list");	
		var list_bz = $.cookie("check_payment_list_fybz");
		if(typeof list == "undefined"){
			list="0";
		}
		if(typeof list_bz == "undefined"){
			list_bz="0";
		}
		var arr = list.split(",");
		var arr_bz=list_bz.split(",");
		//选中
		if(obj.attr("checked")){
			//数组中不存在id，则增加
//			if($.inArray(id, arr)<0){
...
//			}
			...
			if(bzmx){arr_bz.splice($.inArray(bzmx,arr_bz));}
			$(".checkAll").attr("checked",false);
		}
		list = arr.join(",");
		list_bz = arr_bz.join(",");
		$.cookie("check_payment_list",list,{path: '/'});
		$.cookie("check_payment_list_fybz",list_bz,{path: '/'});
		getCheckNum();
	}
