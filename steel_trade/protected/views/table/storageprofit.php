...
<?php if (checkOperation("导出")) {?>
	<div class="con_tit_daoru btn_export"  url="<?php echo Yii::app()->createUrl('table/storageProfitExport');?>">
		<img src="<?php echo imgUrl('daochu.png');?>">导出
	</div>
	<div class="con_tit_duanshu"></div>
<?php }?>
</div>
<form method="post" action="/index.php/table/storageProfit?page=1" url="">
<div class="search_body">
	...
	<div class="shop_more_one_l" style="width: 70px;margin:13px 0 0 8px;;">采购单号：</div>
	...
	<div class="more_one">
		<div class="more_one_l" style="width:75px;margin-left:-5px;">成本价来源：</div>
		 <select name="search[comment]" class='form-control chosen-select forreset'>
	            ...
	            <?php foreach (ProfitCollecting::$source_1 as $key =>$val){?>
...
<div class="total_data">
	<div class="total_data_one">数量：<span><?php echo number_format($totalData["weight"],3);?></span></div>
	<div class="total_data_one">预估销售金额：<span class="color_org"><?php echo number_format($totalData["fee"],2);?></span></div>
	<div class="total_data_one">预估均价：<span ><?php echo $totalData["weight"]!=0?number_format($totalData["fee"]/$totalData["weight"],2):0;?></span></div>
	<div class="total_data_one">预估利润：<span class="color_org"><?php echo number_format($totalData["sales_profit"],2);?></span></div>
</div>
