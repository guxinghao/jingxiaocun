...
<div class="ss_tt_title" style="border-bottom:1px dotted #b1b0b0;">
	<div class="shop_more_one" style="position:relative;margin:6px 40px 0 0;width:250px;">
		<div class="shop_more_one_l" style="width:75px;text-align:right;">销售单：</div>
			...
			<?php if(!$sales){?>
			<span style="float: right;margin:-30px -18px 0 0;width:40px;font-size:15px;color:#fff;text-align:center;border:none;background:#1ca1e4;height:29px;line-height:29px;border-radius:4px;" class="contract_link colorbox" url="<?php echo Yii::app()->createUrl('frmSales/xslist')?>">关联</span>
...
<div class="create_table">
	...
	<table class="table" id="ckck_tb">
		...
		<tr class="">
			...
			<td class=""><input type="text" class="form-control " value="<?php echo $li->amount;?>" readonly></td>
...
function getOutdetail(id){
	$.post("/index.php/frmOutput/xsdetaillist",{"id":id},function(data){
...
$("#save").click(function(){
	...
	$("#ckck_tb tr").each(function(){
		...
		if(num >1 ){
			...
			//if(amount < 0){confirmDialog("请输入第"+(num-1)+"行的出库件数");is_submit = false;return false;}
