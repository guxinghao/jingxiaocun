	...
	<div class="ware_tishi">
			...
			<div class="ware_r">
				<?php foreach ($ware_arr as $li){
					echo '<input type="hidden" id="'.$li['key'].'" value="'.$li['amount'].'" weight="'.number_format($li['weight'],3).'" amount="'.$li['amount'].'">';
...
$("#save").click(function(){
	...
	$("#ckck_tb tr").each(function(){
		...
		if(num >1 ){
			...
			str1 =''+brand+product+texture+rank+length;
			amount_arr += '"'+amount+'",'
			card += '"'+card_id+'",';
			json += '"'+str+'",';
			<?php  if($ware_id && $ware_arr){?>
			var amount1 = Number($("#"+str1).attr("amount1"))+amount;
			var weight1 = Number($("#"+str1).attr("weight1"))+weight;
			$("#"+str1).attr("amount1",amount1);
			$("#"+str1).attr("weight1",weight1);
			<?php }?>
		}
	})
	<?php  if($ware_id && $ware_arr){?>
	var  notfullout = false;
	$(".ware_r  input").each(function(){
			var amount = Number($(this).attr("amount"));
			var weight = Number($(this).attr("weight"));
			var amount1 = Number($(this).attr("amount1"));
			var weight1 = Number($(this).attr("weight1"));
			if(amount != amount1 || weight != weight1){
				notfullout = true;
			}
	})
	if(notfullout){
		confirmDialog("出库单详情不匹配");return false;
	}
	<?php }?>
