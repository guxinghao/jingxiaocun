<script src="<?php echo Yii::app()->request->baseUrl; ?>/js/business.js"></script>
...
<div class="search_body">
		...
		<div style="float:left;">类别：</div>
		 <select name="search[type]" class='form-control chosen-select forreset form_status'>
	         <option value='0' selected='selected'>-全部-</option>
           	 <option <?php echo $search['type']=="XSD"?'selected="selected"':''?>  value="XSD">销售</option>
           	 <option <?php echo $search['type']=="XSTH"?'selected="selected"':''?>  value="XSTH">销售退货</option>
           	 <option <?php echo $search['type']=="CGD"?'selected="selected"':''?>  value="CGD">采购</option>
           	 <option <?php echo $search['type']=="CGTH"?'selected="selected"':''?>  value="CGTH">采购退货</option>
			 <option <?php echo $search['type']=="XSZR"?'selected="selected"':''?>  value="XSZR">销售折让</option>
           	 <option <?php echo $search['type']=="CGZR"?'selected="selected"':''?>  value="CGZR">采购折让</option>
	     </select>
	</div>
	...
	<div class="more_one">
		<div class="more_one_l">金额：</div>
		 <select name="search[money]" class='form-control chosen-select forreset is_yidan'>
	            <option value='0' selected='selected'>-全部-</option>
	            <option <?php echo $search['money']=="1"?'selected="selected"':''?>  value="1">非0</option>	             
           		<option <?php echo $search['money']=="2"?'selected="selected"':''?>  value="2">0</option>
           		<option <?php echo $search['money']=="3"?'selected="selected"':''?>  value="3">大于5</option>
	     </select>
	</div>
</div>
</div>
...
<?php paginate($pages, "business_list")?>
<script>
	$(function(){
		checkStatus();
	})
