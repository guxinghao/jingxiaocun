...
<div class="shop_select_box">
	...
	<div class="shop_more_one">
		...
		<input type="text" id="FrmPurchaseInvoice_fee" class="form-control" value="<?php echo number_format($model->fee, 2, ".", ",");?>" name="FrmPurchaseInvoice[fee]" />
	</div>
	
	<div class="shop_more_one">
		<div class="shop_more_one_l"><span class="bitian">*</span>销票张数：</div>
		<input type="text" id="FrmPurchaseInvoice_capias_amount" class="form-control" value="<?php echo $model->capias_amount;?>" name="FrmPurchaseInvoice[capias_amount]" />
	</div>
	
	<div class="shop_more_one">
		<div class="shop_more_one_l"><span class="bitian">*</span>票号：</div>
		<input type="text" id="FrmPurchaseInvoice_capias_number" class="form-control" value="<?php echo $model->capias_number;?>" name="FrmPurchaseInvoice[capias_number]" />
	</div>
	...
	<div class="shop_more_one">
		<div class="shop_more_one_l"><span class="bitian">*</span>销票时间：</div>
	...
	<table id="cght_tb" class="table">
		<thead>
			<tr>
				...
				<th class="text-center" style="width: 9%;">公司</th>
				<th class="text-center" style="width: 9%;">供应商</th>
				<th class="text-center" style="width: 18%;">产地/品名/材质/规格/长度</th>
				<th class="text-center" style="width: 10%;">销票重量</th>
				<th class="text-center" style="width: 10%;">销票金额</th>
				<th class="text-center" style="width: 9%;">可销票重量</th>
				<th class="text-center" style="width: 9%;">可销票金额</th>
				<th class="text-center" style="width: 10%;">业务员</th>
			</tr>
		</thead>
...
<div class="btn_list" style="margin-bottom: 15px;">
	<button id="submit_btn" type="button" class="btn btn-primary btn-sm" data-dismiss="modal">销票</button>
...
$("#submit_btn").click(function(){
	...
	{
		confirmDialog("销票总金额必须大于0", function(){
			$("#FrmPurchaseInvoice_fee").focus();
		});
		return false;
	}
	*/
	if (!$("#FrmPurchaseInvoice_capias_amount").val() || isNaN(numChange($("#FrmPurchaseInvoice_capias_amount").val())) || parseInt(numChange($("#FrmPurchaseInvoice_capias_amount").val())) <= 0) 
	{
		confirmDialog("销票张数必须大于0", function(){
			$("#FrmPurchaseInvoice_capias_amount").focus();
		});
		return false;
	}
	if (!$("#FrmPurchaseInvoice_capias_number").val()) 
	{
		confirmDialog("请填写票号", function(){
			$("#FrmPurchaseInvoice_capias_number").focus();
		});
		return false;
	}
	...
	{
		confirmDialog("请选择销票时间", function(){
			$("#CommonForms_form_time").focus();
		});
		return false;
	}
	
	confirmDialog3("确认保存修改并销票？", function(){
		if (can_submit) 
		{
			can_submit = false;
