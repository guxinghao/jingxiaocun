...
<?php if (checkOperation("导出")) {?>
	<div class="con_tit_daoru btn_export" url="<?php echo Yii::app()->createUrl('purchaseInvoice/export');?>">
		<img src="<?php echo imgUrl('daochu.png');?>">导出
	</div>
	<div class="con_tit_duanshu"></div>
<?php }?>
	<div class="con_tit_cz">
	<?php if (checkOperation("采购销票:新增")) {?>
		<a href="<?php echo Yii::app()->createUrl("purchaseInvoice/create")?>">
			<button type="button" class="btn btn-primary btn-sm" data-dismiss="modal">采购销票</button>
		</a>
	<?php }?>
	</div>
</div>
		...
		<div class="more_select_box">
			...
			<div class="more_one">
				...
				<select name="search[form_status]" class='form-control chosen-select forreset'>
					...
					<option<?php echo $search['form_status'] == "submited" ? ' selected="selected"' : ''?> value="submited">未销票</option>
					<option<?php echo $search['form_status'] == "capias" ? ' selected="selected"' : ''?> value="capias">已销票</option>
...
//		var text = "确认" + title + "吗？";
	...
	$("#submit").click(function() {
		...
		$.post('/index.php/purchaseInvoice/deleteform', 
		{
			'id' : list_id,
