...
<div class="con_tit">
	...
	<?php if(checkOperation("转库单:新增") && $sales->confirm_status == 0){?>
	<a href="<?php echo $createUrl;?>" style="text-decoration: none;">
		<button type="button" class="btn btn-primary btn-sm" data-dismiss="modal">新建转库单</button>
	</a>
	<?php }?>
	</div>
...
<div class="search_body">
	...
	<div class="shop_more_one1">
		 <div style="float:left;">转库状态：</div>
		 <select name="search[form_status]" class='form-control chosen-select forreset'>
	            <option value='0' selected='selected'>-全部-</option>	             
           		 <option <?php echo $search['form_status']=="unsubmit"?'selected="selected"':''?>  value="unsubmit">未转库</option>
           		 <option <?php echo $search['form_status']=="submited"?'selected="selected"':''?>  value="submited">已转库</option>
