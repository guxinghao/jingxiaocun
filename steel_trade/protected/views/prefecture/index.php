<div class="con_tit">	
	<div class="con_tit_cz">
		<a href="<?php echo Yii::app()->createUrl('prefecture/create',array('page'=>$_REQUEST['page']))?>">
		<button type="button" class="btn btn-primary btn-sm" data-dismiss="modal">新建专区</button>
		</a>
	</div>
</div>
<style>
		...
		<tbody>
			<?php $i=0;
				foreach ($items as $item){
					$manage_url=Yii::app()->createUrl('prefecture/manage',array('id'=>$item->id));
					$edit_url = Yii::app()->createUrl('prefecture/update',array('id'=>$item->id));
			?>
			<tr id="<?php echo $item->id?>" class="data">
				<td class='table_cell_first'>
					<div class="cz_list_btn">
						<a class="update_b" title="管理"  href="<?php echo $manage_url;?>"><i class="icon icon-th-list"></i></a>
						<a class="update_b" title="修改"  href="<?php echo $edit_url;?>"  style="margin-left:5px"><i class="icon icon-edit"></i></a>
	...
	$(function(){
		$(".del_user").click(function(){
			...
			confirmDialogWithCallBack("确定要删除吗",function(){
				$.post("<?php echo Yii::app()->createUrl('prefecture/delete')?>", {
