<div class="con_tit">
	...
	<?php if($search['input_type']=="ccrk"){?>		
	<?php }else{?>
		<a href="<?php echo Yii::app()->createUrl('input/create',array('id'=>'0','type'=>'purchase','fpage'=>$_REQUEST['page']))?>">
		<button type="button" class="btn btn-primary btn-sm create_btn" data-dismiss="modal">采购入库</button>
		</a>
		<a href="<?php echo Yii::app()->createUrl('input/createByReturn',array('fpage'=>$_REQUEST['page']));?>">
		<button type="button" class="btn btn-primary btn-sm create_btn" data-dismiss="modal">销售退货入库</button>
		</a>
	<?php }?>
	</div>
</div>
...
<form method="post" action="/index.php/input/index?page=1">
<div class="search_body">
	<div class="srarch_box">
		<img src="<?php echo imgUrl('search.png');?>">
		<input placeholder="入库/采购/退货单号" id="srarch" class="forreset" value="<?php echo $search['keywords']?>" name="search[keywords]">
	</div>
...
?>
 ...
 <div class="total_data" > 
	...
	<!--  -><div class="total_data_one">单数：<span class="color_org"><?php echo $totalData["total_num"];?></span></div>-->
</div>
...
<script>
	    ...
	    $(function(){
		    $('.submit_form').click(function(e){
			    	...
			    	{
				    	var flag=true;
				    	var thisid= $(this).parent().parent().find(".form_sn").attr('thisid');
				    	$.ajaxSetup({async:false});
				    	$.get('/index.php/input/beforeCheck/'+thisid,{},function(data){
					    	if(data!=1)
					    	{
						    	if(data=='error'){confirmDialog('数据错误');}else{
							    	confirmDialog(data);
						    	}
						    	flag= false;
					    	}
