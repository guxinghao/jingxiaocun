<div class="con_tit">
	...
	<div class="con_tit_cz">
		<a href="<?php echo Yii::app()->createUrl('input/create',array('id'=>'0','type'=>'dxrk','fpage'=>$_REQUEST['page']))?>">
		<button type="button" class="btn btn-primary btn-sm create_btn" data-dismiss="modal">新建代销入库</button>
		</a>
	</div>
</div>
<style>
		...
		<div class="more_one_l">入库状态：</div>
		 <select name="search[form_status]" class='form-control chosen-select forreset'>
	            <option value='0' selected='selected'>-全部-</option>	             
           		 <option <?php echo $search['form_status']=="unsubmit"?'selected="selected"':''?>  value="unsubmit">未入库</option>
           		 <option <?php echo $search['form_status']=="approve"?'selected="selected"':''?>  value="approve">已入库</option>
...
<script>
	    ...
	    $(function(){
		    $('.submit_form').click(function(e){
		    	 	...
		    	 	var type=$(this).attr('is_input');
		    	 	if(type=='input')
			    	{
				    	var flag=true;
				    	var thisid= $(this).parent().parent().find(".form_sn").attr('thisid');
				    	$.ajaxSetup({async:false});
				    	$.get('/index.php/input/beforeCheck/'+thisid,{'type':'dx',},function(data){
					    	if(data!=1)
					    	{
		    ...
		    $('.create_btn').click(function(e){
				var result=checkAuthority('代销入库单:新增');
				if(result=='no')
				{
// 					alert('您没有权限执行此操作');
					e.preventDefault();
