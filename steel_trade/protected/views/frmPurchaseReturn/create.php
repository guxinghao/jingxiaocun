...
<div class="shop_select_box">
	<div class="shop_more_one">
		...
		<div id="ttselect" class="fa_droplist">
			...
			<input type='hidden' id='combval' value="" name="PurchaseReturn[title_id]" />
		</div>
	</div>
	<div class="shop_more_one">
		...
		<div id="wareselect" class="fa_droplist">
			...
			<input type='hidden' id='comboval' value="" name="PurchaseReturn[supply_id]" />
		</div>
	</div>
	<div class="shop_more_one">
	...
	<div class="shop_more_one">
		...
		<div id="warehouseselect" class="fa_droplist">
			...
			<input type='hidden' id='comboval3' value="" class="wareinput" name="PurchaseReturn[warehouse_id]"/>
		</div>
	</div>
	<div class="shop_more_one">
		...
		<div class="search_date_box" style="margin-top:0px;background-position:155px 8px;">
			<input type="text"  name="CommonForms[form_time]" class="form-control form-date date start_time date_jquery" placeholder="选择日期"  value="<?php echo date("Y-m-d");?>">
		</div>
	</div>
	<div class="shop_more_one">
		<div class="shop_more_one_l">预计退货日期：</div>
		<div class="search_date_box" style="margin-top:0px;background-position:155px 8px;">
			<input type="text"  name="PurchaseReturn[return_data]" class="form-control form-date date fs_date date_jquery"  placeholder="选择日期"  >
		</div>
	</div>
	<div class="shop_more_one">
		<div class="shop_more_one_l">车牌号：</div>
		<input type="text"  class="form-control con_tel" name="PurchaseReturn[travel]" id="traver" placeholder="多个用空格或逗号隔开">
	</div>
	<div class="shop_more_one">
	...
	<div class="shop_more_one">
		<div class="shop_more_checkbox">
			 <label class='radio-inline'> <input class="check_box" type="checkbox" name="PurchaseReturn[is_yidan]" value="1"> 乙单 </label>
		</div> 
	</div>
	<div class="shop_more_one">
		<div class="shop_more_one_l">备注：</div>
		<input type="text" class="form-control tit_remark" placeholder="" name="CommonForms[comment]">
	</div>
</div>
    ...
    <tbody>
   		...
   		<tfoot>
	    	<tr class="tablefoot">
				...
				<td style="width:5%;"></td>
				<td style="width:8%;"></td>
				<td style="width:8%;"><span class="total_amount">0</span></td>
				<td style="width:9%;"><span class="total_weight">0</span></td>
				<td style="width:10%;"></td>
...
$("#save_sub").click(function(){
		...
		$("#cght_tb tr").each(function(){
			...
			if(num >1){
				...
				if(td_num == ''){confirmDialog("请输入退货件数");$(this).find(".td_shop_num").addClass("red-border");is_submit = false;return false;}
				if(parseInt(td_num) > parseInt(td_surplus)){confirmDialog("退货件数大于可退件数");$(this).find(".td_shop_num").addClass("red-border");is_submit = false;return false;}
...
<script>
	...
	$(document).on('change','.td_shop_num',function(){
			...
			if(td_money > 0){
				$(this).parent().parent().find('.td_price').val(fmoney(td_money*amount,3));
			}
			setTotalamount();
			setTotalweight();
			setTotalprice();
