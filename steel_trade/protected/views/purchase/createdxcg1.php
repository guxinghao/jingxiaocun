...
<input type="hidden" id="tr_num" value="1">
    ...
    <tbody class="forinsert" id="forinsert">
    	...
    	?>
    		...
    		<td class="">
    			<input type="hidden"  name="td_brands[]" value="<?php echo $each->brand_id?>" class="form-control yes td_brand">
    		...
    		<td class="">
    			...
    			<input type="hidden" name="td_products[]" value="<?php echo $each->product_id?>" class="form-control yes td_product">
	    		<input type="text" readonly name="td_products_name[]" value="<?php echo $each->product_name;?>" class="form-control">
    		</td>
    		<td class="">
    			<input type="hidden"  name="td_textures[]" value="<?php echo $each->texture_id?>" class="form-control yes td_texture">
	    		<input type="text" readonly name="td_textures_name[]" value="<?php echo $each->texture_name?>" class="form-control">
    		</td>   
    		<td class="">
    			<input type="hidden"  name="td_ranks[]" value="<?php echo $each->rank_id?>" class="form-control yes td_rank">
    		...
    		<td >
    			...
    			<input type="hidden"  name="td_weight[]"  value="<?php echo round($each->weight,3);?>" style="" class="form-control td_total_weight" >
    		</td>
...
<div class="btn_list" style="margin-top:30px;">
	<button type="submit" class="btn btn-primary btn-sm blue" data-dismiss="modal" style="float:right;margin-right:20px" id="submit_btn">下一步</button>
...
<div class="search_body search_background">
	...
	<div class="more_select_box" style="top:40px;left:-640px;">
		<div class="more_one">
...
// 			confirmDialog(contact_id);
	    ...
	    var unit_weight=0;
		$(document).on('change','.td_num',function(e){
			...
			{
				var objj=$(this);
				confirmDialog('购买的件数必须小于最大可购买件数'+td_max_num,function(){$(objj).addClass('red-border')});
				$(this).val(td_max_num);
				return false;
			}
			//获取件重			
...
/****------------------------------销售列表有关》》---------------*****/
	...
	//选择，填充数据，撤销，恢复数据
	$(document).on('click','.selected_sales',function(e){
			 e.stopPropagation();
			var that=$(this);
	...
	$(function(){
		...
		$('#combo5').combobox(array2, {imageUrl : "<?php echo imgUrl('dropdown.png');?>"},"saleselect","comboval5");
		$('#combo6').combobox(array, {imageUrl : "<?php echo imgUrl('dropdown.png');?>"},"cusselect","comboval6");
