...
<!-- 	</div> -->
	...
	<div class="shop_more_one "  >
		...
		<input type="text"  name="FrmPurchase[shipment]" class="form-control " readonly  value="<?php echo $purchase->shipment?>" id="shipment" style="width:145px;height:33px;" placeholder=""  >
...
<input type="hidden" id="tr_num" value="<?php echo count($details)?>">
    ...
    <tbody class="forinsert" id="forinsert">
    	...
    	<tr class="<?php echo $i%2==0?'selected':''?>">
    		...
    		<td class="">
					<input type="text" id="<?php echo "bbcombobrand".$i?>" readonly   value="<?php echo DictGoodsProperty::getProName($each->brand_id)?>" class="form-control" />
...
// 		var result = checkTravel(transfer);
// 		if(result != 1){confirmDialog(result);return false;}
		...
		$("#cght_tb tbody tr").each(function(){
			...
			if(brand==''&&td_amount==''&&td_weight==''&&td_price=='')
			{				
