...
class DictGoodsPropertyData extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id, property_type, name', 'required'),
			array('id, last_update', 'numerical', 'integerOnly'=>true),
			array('property_type, name, short_name, code, std, reserve', 'length', 'max'=>45),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, property_type, name, short_name, code, std, last_update, reserve', 'safe', 'on'=>'search'),
		);
	}
