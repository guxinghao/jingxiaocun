 ...
 * @property string $cus_number
 * @property string $sup_number
 * @property string $dj_number
...
class DictCompanyData extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('is_supply, is_pledge, is_customer, is_logistics, is_dx, is_gk, is_warehouse, created_at, created_by, level, pledge_length, priority, su_priority, loan_priority, warehouse_id', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>50),
			array('short_name, code, cus_number, sup_number, dj_number', 'length', 'max'=>45),
			array('fee', 'length', 'max'=>11),
			array('pledge_rate', 'length', 'max'=>7),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, short_name, is_supply, is_pledge, is_customer, is_logistics, is_dx, is_gk, is_warehouse, code, created_at, created_by, fee, level, pledge_length, pledge_rate, priority, su_priority, loan_priority, cus_number, sup_number, dj_number, warehouse_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'cus_number' => 'Cus Number',
			'sup_number' => 'Sup Number',
			'dj_number' => 'Dj Number',
	...
	public function search()
	{
		...
		$criteria->compare('sup_number',$this->sup_number,true);
		$criteria->compare('dj_number',$this->dj_number,true);
