...
class DictCompany extends DictCompanyData
{
	...
	public static function getVendorList($type,$name="")
	{
		...
		if($change == "is_supply"){
			$vendors = DictCompany::model()->findAllByAttributes(array($change => '1'),array("select"=>"id,code,name","order"=>"su_priority ASC"));
		}else{
			$vendors = DictCompany::model()->findAllByAttributes(array($change => '1'),array("select"=>"id,code,name","order"=>"priority ASC"));
		}
