...
class PHPExcel
{
	...
	*/
    ...
    // $title 导出文件内容的第一行
    // $content 导出文件的内容
    public static function ExcelExport($name,$title,$content){
		...
		foreach($title as $k=>$v){
			$objPHPExcel->getActiveSheet()->setCellValue($word[$k].'1',$v);
		}		
		//设置内容
		$num = 2;
		foreach($content as $each){
			foreach($each as $k=>$v){
				$objPHPExcel->getActiveSheet()->setCellValue($word[$k].$num,$v,PHPExcel_Cell_DataType::TYPE_STRING);
			}
			$num++;
		}
    ...
    // $title 导出文件内容的第一行
    // $content 导出文件的内容
    public static function BusinessExcelExport($name,$title,$content,$sheet){
    	...
    	$objPHPExcel->getActiveSheet()->setTitle('t_Schema'); //设置工作表名称
    	//设置内容
    	$num = 1;
    	foreach($sheet as $li){
    		foreach($li as $k=>$v){
    	...
    	//创建第二个工作表
    	$msgWorkSheet = new PHPExcel_Worksheet($objPHPExcel, 'Page1'); //创建一个工作表
    	...
    	foreach($content as $each){
    		foreach($each as $k=>$v){
    			$objPHPExcel->getActiveSheet()->setCellValueExplicit($word[$k].$num,$v,PHPExcel_Cell_DataType::TYPE_STRING);
    		}
    		$num++;
    	}
    	//设置单元格F列（编码）为文本格式，实验发现如果格式不一致，以第一个为准，其余的不一致的均报找不到的错误
    	$objPHPExcel->getActiveSheet()->getStyle('F'.$j)->getNumberFormat()->setFormatCode("@");
